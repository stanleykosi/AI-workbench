...
async def predict(
    ...
    try:
        ...
        # Determine the best approach based on model type
        model_class_name = model.__class__.__name__
        is_time_series_model = model_class_name in ["LstmModel", "GruModel", "TransformerModel"]
        
        if is_time_series_model and hasattr(model, 'forecast'):
            # Time series models (LSTM, GRU, Transformer) use forecast method
            try:
                print(f"📊 Using forecast method for time series model: {model_class_name}")
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    # Get time_steps from model config if available
                    time_steps = getattr(model.config, 'time_steps', 60)
                    min_data_points = max(time_steps, 10)  # At least time_steps or 10
                    ...
                    # Use the last data points for prediction
                    recent_data = training_data.tail(max(min_data_points, 100))
                    print(f"📊 Using {len(recent_data)} data points for {model_class_name} prediction")
            ...
            # Traditional ML models (Random Forest, XGBoost, ARIMA) use inference method
            next_prediction = None
