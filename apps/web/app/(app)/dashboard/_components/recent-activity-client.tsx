/**
 * @description
 * This component displays recent activity using React Query for caching.
...
function RecentActivityDisplay({ activities }: { activities: ActivityItem[] }) {
...
export function RecentActivityClient() {
  const { data: recentActivity, isLoading, error, isError } = useRecentActivity(10);
  ...
  if (isError) {
    ...
    // Fallback to showing empty state if there's an error
    return (
      <RecentActivityDisplay activities={[]} />
    );
  }
